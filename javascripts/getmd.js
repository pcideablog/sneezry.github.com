...
function showlist(list){
	...
	for(var i = list.data.length; i > 0; i--){
		txt += '<postlist><a href="/#!/' + list.data[i-1].name.replace(/-/g, '/') + '">' + list.data[i-1].name.split('-')[list.data[i-1].name.split('-').length-1].replace(/_/g, ' ') + '</a><span class="disqus_count"><a href="/' + list.data[i-1].name.replace(/-/g, '/') + '#disqus_thread"></a></span></postlist>';
	}
	loading.style.display = 'none';
